...
**ничего не выдумано**. Перед публичным запуском владелец должен подтвердить
и, при готовности, заменить обтекаемые формулировки конкретикой:

- **Хостинг данных** — где физически хранятся данные и есть ли
  **AV-Vertrag** (Auftragsverarbeitung). Сейчас: «Hosting … folgt mit der
  Pilotphase».
- **Impressum** — реальная правовая форма и адрес Memora. Сейчас: «Impressum
  … folgt», в футере «Impressum & Datenschutz folgen».
- **Сроки хранения/удаления** после Abschluss — реальные Löschfristen. Сейчас:
  «richten sich nach den gesetzlichen Vorgaben … in der Pilotphase».
- **«Zertifizierung»** — допустимо ли это слово вообще, даже как «folgt».
  Сейчас: «mögliche Zertifizierungen folgen» — проверить, не вводит ли в
  заблуждение, пока сертификации нет.
